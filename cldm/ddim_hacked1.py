...
class DDIMSampler(object):
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      ...
                      dynamic_threshold=None, scale_num=1):
        model_input = x
        if self.model.channels == 9:
        ...
        if scale_num == 2:
            ...
            c_in = {
                ...
                "c_crossattn": [torch.cat([
                    c["c_crossattn"][0],
                    c["c_crossattn"][0],
                    unconditional_conditioning["c_crossattn"][0],
                    unconditional_conditioning["c_crossattn"][0]
                     ])],
                "c_concat": [torch.cat([
                    c["c_concat"][0],
                    unconditional_conditioning["c_concat"][0],
                    c["c_concat"][0],
                    unconditional_conditioning["c_concat"][0]
                ])],
            }

            text_cfg_scale, image_cfg_scale = unconditional_guidance_scale
            out_cond, out_text_cond, out_img_cond, out_uncond = self.model.apply_model(x_in, t_in, c_in).chunk(4)
            model_output =  out_uncond
            # model_output = out_uncond + text_cfg_scale * (out_cond - out_img_cond) + image_cfg_scale * (out_img_cond - out_uncond)
